...
#pragma warning restore NUnit2045
        ...
        private const string TestMap = "/Maps/_NF/Outpost/frontier.yml";

        /// <summary>
        ///     Loads the default map, runs it for 5 ticks, then assert that it did not change.
        /// </summary>
        [Test]
        public async Task LoadSaveTicksSaveKal()
        {
            await using var pair = await PoolManager.GetServerClient();
            var server = pair.Server;
            ...
            // Load frontier.yml as uninitialized map, and save it to ensure it's up to date.
            server.Post(() =>
            {
        ...
        ///     similar to <see cref="LoadSaveTicksSaveKal"/> and <see cref="SaveLoadSave"/>, but neither of these
        ///     caught the mentioned bug.
        /// </remarks>
        [Test]
        public async Task LoadTickLoadKal()
        {
            await using var pair = await PoolManager.GetServerClient();
            var server = pair.Server;
