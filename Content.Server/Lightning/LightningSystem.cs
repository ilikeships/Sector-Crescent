...
public sealed class LightningSystem : SharedLightningSystem
{
    ...
    public void ShootRandomLightnings(EntityUid user, float range, int boltCount, string lightningPrototype = "Lightning", int arcDepth = 0, bool triggerLightningEvents = true)
    {
        ...
        while(shootedCount < boltCount)
        {
            ...
            var curTarget = EntityManager.GetComponent<LightningTargetComponent>(targets[count]);
            ...
            if (arcDepth - curTarget.LightningResistance > 0)
            {
                ShootRandomLightnings(targets[count].Owner, range, 1, lightningPrototype, arcDepth - curTarget.LightningResistance, triggerLightningEvents);
            }
            shootedCount++;
        }
    }
